    ...
    <div className="">
      ...
      <div className="m-4 mt-28 flex flex-col sm:flex-row gap-2 sm:gap-4">
        <div className="sm:w-1/2 max-w-lg">
          {items.map((product) => (
            <CartProduct key={product.product} productId={product.product} name={product.name} img={product.img} price={product.price} brand={product.brand} color={product.color} />
          ))}
        </div>
